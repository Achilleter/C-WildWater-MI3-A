...
Noeud *creernoeud(const char *id) { // Fabrique le Noeud à partir de l'id Usine
...
Noeud *verifFichier(const char *fichier, const char *id_usine, AVLeak **index, float *vol_init) {//Ouverture et verification du fichier
	...
	*vol_init = 0;
	...
	while (fscanf(f, "%63[^;];%63[^;];%63[^;];%63[^;];%63[^\n]\n", c1, c2, c3, c4, c5) == 5) {
		...
		if (amont && aval) {
			ajouteEnfants(amont, aval, i.fuites);
...
float calculerFuites(Noeud *n, float volume) { // Parcours la totalité des noeuds pour calculer les fuites
	if (!n || volume <= 0) return 0;

	int nb = nbrEnfant(n);
	if (nb == 0) return 0;

	float part = volume / nb;
	float total_perdu = 0;

	for (Enfant *e = n->enfants; e; e = e->suivant) {
		float perdu = part * e->fuite / 100.0f;
		total_perdu += perdu;

		float restant = part - perdu;
		total_perdu += calculerFuites(e->noeud, restant);
	}
